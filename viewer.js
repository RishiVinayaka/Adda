...
const hideRightToolbar = () => {
  const rightToolBar = document.getElementById("toolbarViewerRight");
...
const addNoCopyContentScript = () => {
  const scriptElement = document.createElement('script');
  scriptElement.src = '/images/drm/nocopy-content.js?r=1';
  document.head.appendChild(scriptElement);
}

const addCtrlAndCmdDisableScript = () => {
...
const isADDA = window.location.href.includes('adda247');
const params = new URLSearchParams(window?.location?.search);
const docFile = params?.get("file");
if (!(docFile?.includes("lc") || docFile?.includes("dpp")) && isADDA) {
        console.log('All 4 restriction functions skipped');
}
...
const PDFViewerApplication = {
  ...
  async open(args) {
    ...
    return loadingTask.promise.then(pdfDocument => {
      ...
      // custom message show
      this.pdfViewError(reason.message)
  ...
  pdfViewError(message, moreInfo) {
    ...
    // getting the HTML elements to show custom errors
    ...
    // console.log(errorWrapperConfig);
  ...
  try {
    if (file) {
      PDFViewerApplication.open({
        ...
        setTimeout(() => {
          var pNo = parseInt(pageNo) ? parseInt(pageNo) : 1;
          PDFViewerApplication.pdfViewer.currentPageNumber = pNo; // Ensure '5' is parsed as a number
          _saveEbookStatus(pNo);
        }, 100); 
      });
    } else {
...
function _saveEbookStatus(page) {
  const isADDA = window.location.href.includes('adda247');
  const params = new URLSearchParams(window?.location?.search);
  const docFile = params?.get("file").split('.')[0];
  const pkgId = docFile.split("_")[0]
  const eId = docFile.split("_")[1]
  const TestServerDomains = ["localhost","dev","qa","staging","alpha"]
  const isTestServer = TestServerDomains.some(domain => window.location.href.includes(domain)) ? 0 : 1;
  const urls = isADDA ? {
    0:'https://devstore.adda247.com/api/v1/ebooks/saveStatusOfEbook',
    1:'https://store.adda247.com/api/v1/ebooks/saveStatusOfEbook',
  } : {
    0:'https://stagingstore.sankalpbharat.com/api/v1/ebooks/saveStatusOfEbook',
    1:'https://store.sankalpbharat.com/api/v1/ebooks/saveStatusOfEbook',
  }
  const headers = { "x-auth-token": "fpoa43edty5", "x-jwt-token": _getCookie("cp_token")}
  const url = `${urls[isTestServer]}/${pkgId}/${eId}?pageNumber=${page}&src=aweb`;
...
if (typeof window !== "undefined" && window["pdfjs-dist/build/pdf"]) {
  ...
  pdfjsLib = require("../build_latest/pdf.js");
}
...
const defaultOptions = {
  ...
  workerSrc: {
    value: "./images/pdfjs_latest/build_latest/pdf.worker.js",
    kind: OptionKind.WORKER
  }
};
{
  defaultOptions.defaultUrl = {
    // value: "/images/pdfjs_latest/web/docs/" + "compressed.tracemonkey-pldi-09.pdf",
    // value: "/images/pdfjs_latest/web/docs/" + "samplepdf.doc",
    // value: "/images/pdfjs_latest/web/docs/" + "4970_26_not_opening.doc",
    // value: "/images/pdfjs_latest/web/docs/" + "25467_11127_not_opening.doc",
    // value: "/images/pdfjs_latest/web/docs/" + "25467_11128_not_opening.doc",
    // value: "/images/pdfjs_latest/web/docs/" + "25467_11129_not_opening.pdf",
    // value: "/images/pdfjs_latest/web/docs/" + "25467_11131_opening.doc",
    value: window?.location?.origin + '/' + Object.fromEntries((new URLSearchParams(window?.location?.search)))?.file,
    kind: OptionKind.VIEWER
  };
  ...
  defaultOptions.sandboxBundleSrc = {
    value: "../build_latest/pdf.sandbox.js",
    kind: OptionKind.VIEWER
  };
}
